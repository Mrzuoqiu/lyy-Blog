...
class db {
    ...
    //连接数据库的操作
    //不再使用PHP中的mysql语句来完成数据库的连接、查询、删除、修改等操作
    //默认的方式:mysql_connect连接数据，mysql_select_db选择数据库 mysql_error错误信息(连接数据库mysql_connect返回的错误信息)
    //mysql_query执行SQL语句，mysql_fetch_row 获取一条数据
    //这里，使用的是PDO的接口来完成对数据的操作CURD
    ...
    //返回数据库表的名字，当前要操作的表是哪个？
    public function table($table) {
        return '`' . @$this->config['tablepre'] . $table . '`';
    }
    //对用户提交的数据在插入数据库之前，对单引号，双引号，反斜杠，和null做一下转义处理，在前面添加反斜杠
    ...
    //用户的查询条件，循环，并 key = \'value\'这种形式保存下,然后返回一个在查询条件数组都添加and字符串的数组。
